 ...
 */
let _db = null;
const _dbName = 'ims_db'; // test_db数据库
const _dbVersion = 1;
...
const initIndexedDB = () => {
  ...
  request.onerror = function (e) {
    console.error('error to open DB', e);
  };
  request.onsuccess = function (e) {
    console.debug("success to open DB");
    _db = e.target.result;
    indexedDB.db = _db;
  };
...
const _store = (storeName, mode = 'readwrite') => {
  return new Promise((resolve) => {
    let objectStore = _db.transaction([storeName], mode).objectStore(storeName);
    resolve(objectStore);
  });
};
