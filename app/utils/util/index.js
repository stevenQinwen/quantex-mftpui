...
const toTreeData = (dataList = [], config = {}) => {
  ...
  for (let i = 0; i < dataList.length; i++) {
    ...
    if (row.id == row.pId || _.isNil(row.pId)) {
      items.push(item);
    }
    idMap[row.id] = item;
  }
...
const buildFormData = (formData = {}, options = {}) => {
  let _formData = {};
  ...
  for (let entry of entries) {
    ...
    // 增加配置项: 支持 permitPeriod[${i}].startTime-endTime
    // { permitPeriod: [{ startTime-endTime: ['100000', '203040'] }] } =>
    // { permitPeriod: [{ startTime: '100000', endTime: '203040' }] }
    if (opt.expand && Array.isArray(value)) {
      value = value.map((va, index) => {
        let childOpt = opt.children ? opt.children[index] : {};
        return buildFormData(va, childOpt);
      });
    }
    if (fields.length === 1) {
      vals.push(value);
    ...
    fields.forEach((field, i) => {
      ...
      if ($$index > -1) {
        ...
        if (dataType === "array" && tempValue instanceof Array) {
          vals[i] = tempValue.map((temp, index) => {
            let childOpt = opt.children ? opt.children[index] : {};
            // 如果元素为对象，则进行递归编译子参数
            if (temp instanceof Object) {
              temp = buildFormData(temp, childOpt);
            }
            return temp;
          });
        } else {
      ...
      _formData[field] = typeof vals[i] == 'string' ? vals[i].trim() : vals[i];
    });
  }
  return _formData;
};
